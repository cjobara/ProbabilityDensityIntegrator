...
RawDensity=zeros(1140, 3, NumFiles);

NormDensity=zeros(1140, 3, NumFiles);

r_bar_raw=zeros(NumFiles+1,3);
dI_dr_raw=zeros(NumFiles+1,3);

r_bar_cyto=zeros(NumFiles+1,3);
r_bar_total=zeros(NumFiles+1,3);
dI_dr_cyto=zeros(NumFiles+1,3);
dI_dr_total=zeros(NumFiles+1,3);
...
for i=1:NumFiles
    ...
    %If any color is all NaN, remove it before assigning it to the array or
    %they will not be the correct size
    if any(sum(isfinite(RawProbDensity),1)==0)
        NullSet=find(sum(isfinite(RawProbDensity),1)==0);
        UseSet=setdiff(1:size(RawProbDensity,2),NullSet);
        RawProbDensity=RawProbDensity(:,UseSet);
        StrappedProbDensity=StrappedProbDensity(:,UseSet);
    end
